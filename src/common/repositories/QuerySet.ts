...
export class QuerySet<T extends BaseEntity> {
    ...
    leftJoinAndSelect(property: string, alias: string): this {
        this.queryBuilder.leftJoinAndSelect(`${this.entityName}.${property}`, alias);
        return this;
    }
    filterById(entityId: string | undefined): this {
        this.queryBuilder = this.queryBuilder.andWhere(`id = :${this.entityName}Id`, {
    ...
    filterByInstances(entities: T[]): this {
        return this.filterByIds(entities.map((entity) => entity.id as unknown as string));
    }
    ...
    paginate(pagination: {count: number | undefined; lastRetrievedId: any}, sort = SortType.ASC): this {
    ...
    whereField(field: string, value: string | number | Record<string, Date>): this {
